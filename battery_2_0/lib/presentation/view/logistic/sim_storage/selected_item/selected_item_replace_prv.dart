...
class SelectedItemReplacePrv extends StatelessWidget {
  final Map itemData;
  final List allItems;
  const SelectedItemReplacePrv(
  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocProvider(
      providers: [
        BlocProvider<SimItemReplaceBloc>(create: (context) => SimItemReplaceBloc()..add(InitEvent(initData: Map.from(itemData)))),
      ],
      child: Scaffold(
        appBar: AppBar(
          elevation: 1,
        ...
        body: ProgressHUD(
          ...
          child: Builder(builder: (context) {
            ...
            String palletSize = replaceState['replace']['pallet_size'] == 'big' ? 'большой' : 'стандартный';
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _enterValue('ячейка', replaceState['replace']['cell'].toString(), () async {
                        List roomates;
                        ProgressHUD.of(context)?.showWithText('загрузка');
                        await SimItemsImpl().getCells(context, replaceState['replace']['place'].toString()).then((value) {
                          ProgressHUD.of(context)?.dismiss();
                            ...
                            children: [
                              Switch(
                                ...
                                onChanged: (bool value){
                                  value ?
                                    context.read<SimItemReplaceBloc>().add(const ChangePalletEvent(data: 'yes')) : 
                                    context.read<SimItemReplaceBloc>().add(const ChangePalletEvent(data: 'no'));
                                  // replace.palletSize == 'big' ?
                                  //   {
                                  //     replaceState['replace']['pallet_size'] = 'standart',
                                  //     context.read<SimItemReplaceBloc>().add(ReplaceEvent(replaceData: replaceState))
                                  //   } : 
                                  //   {
                                  //     replaceState['replace']['pallet_size'] = 'big',
                                  //     context.read<SimItemReplaceBloc>().add(ReplaceEvent(replaceData: replaceState))
                                  //   };
                                }
                              ),
                              ...
                              Expanded(
                                child: Text(
                                  ...
                                  '${merge ? 'объединяем' : 'оставляем'} на $palletSize паллет', 
                                  style: firm14
                                ),
                              )
                            ],
